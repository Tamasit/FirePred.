  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const filterData = jsonData.filter(
        (item) => item.acq_date && item.acq_date.includes("2013")
      );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div
        style={{
          display: "flex",
          justifyContent: window.innerWidth < 600 ? "flex-end" : "flex-end",
          marginLeft: window.innerWidth < 600 ? "0" : "200px", // Adjust the margin for mobile screens
          padding: "0 20px", // Adjust padding for better spacing
        }}
      >
        <Link
          to="/YearPage/Year2014"
          className="link-to-year2014 hover:text-red-500 transition-colors text-l"
        >
          ปี 2014 {">"}
        </Link>
      </div>
      ...
      >
        <span className="text-blue-700">ข้อมูลสถิติการเกิดไฟในปี 2013</span>
      </h1>
      <div
        style={{
          ...containerStyle,
